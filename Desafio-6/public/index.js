...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sendMessageButton.addEventListener("click", (e) => sendInputData(e, e.type, "click"));

  // Imprimir mensajes
  socket.on("user", (data) => {
    chatMessages.innerHTML = data.map((messages) => {
        if (messages.user === currentUser) {
          return `<div class="flex p-1 gap-1 bg-slate-100 backdrop-opacity-50 shadow rounded mt-2"><span class="text-green-500">${messages.date} User${messages.user} (You): </span><p>${messages.msg}</p></div>`;
        } else {
          return `<div class="flex p-1 gap-1 bg-slate-100 backdrop-opacity-50 shadow rounded mt-2"><span class="text-red-500">${messages.date} User${messages.user}: </span><p>${messages.msg}</p></div>`;
        }
      }).join(" ");
  });
